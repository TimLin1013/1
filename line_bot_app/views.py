                ...
                if isinstance(event.message, TextMessage):
                    ...
                    if mtext == '我的帳本':
                        ...
                        if len(mtext[2:]) > 200:  # 判斷群組名稱有沒有超過200字
                            reply_message = '群組名稱超過200字，請重新輸入'
                            line_bot_api.reply_message(event.reply_token, TextMessage(text=reply_message))
                        else:
                            reply_message = func.CreateGroup(mtext, user_id, user_name)
                            line_bot_api.reply_message(event.reply_token, TextMessage(text=reply_message))
                    elif mtext == '加入群組':
                        reply_message = '請輸入您想要加入的群組代碼'
                        line_bot_api.reply_message(event.reply_token, TextMessage(text=reply_message))
                    elif mtext[:6] == '<加入群組>':
                        reply_message = func.JoinGroup(mtext, user_id, user_name)
                        line_bot_api.reply_message(event.reply_token, TextMessage(text=reply_message))
        return HttpResponse()
    else:
        return HttpResponseBadRequest()
@csrf_exempt
def lift_add(request):
    if request.method == 'GET':
        return render(request,'Line_lift_add.html')
    else:
        return HttpResponseBadRequest()